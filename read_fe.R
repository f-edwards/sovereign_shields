...
fe <- read_csv("./data/fe_2_14_20.csv") %>% 
  filter(!(is.na(`Unique ID`))) # remove last row warning

# .... make names more user-friendly
names(fe)<-c("id", "name", "age", "sex", "race", "imp_race", "imp_prob",
             "URL", "death_date", "loc_address", "loc_city", "loc_state", 
             "loc_zip", "loc_county", "loc_full_address", "Latitude", "Longitude", 
             "agency", "cause_of_death","cause_description", "official_disposition", 
