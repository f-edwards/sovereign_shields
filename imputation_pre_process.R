...
fe_imp_dat<-fe_new%>%
  ...
  select(id, year, FIPS_county, FIPS_block, 
         loc_state, surname, age, sex, race, 
         fe_cause_of_death)%>%
...
pct_cnty_pop<-pop_cnty%>%
  ...
  group_by(year, FIPS_county, race_ethn)%>%
  ...
  select(year, FIPS_county, race_ethn, pct_pop)
...
### format for mice
fe_imp_dat_join<-fe_imp_dat_join%>%
  select(id, year, FIPS_county, loc_state, age, sex, race, fe_cause_of_death,
